...
class TestScoringService:
    ...
    def test_high_competition_reduces_score(self) -> None:
        svc = ScoringService()
        score_low_comp = svc.score(
            self._make_low_comp(), self._make_rich_geo(), "kuliner_kopi"
        ).score
        score_high_comp = svc.score(
            self._make_high_comp(), self._make_rich_geo(), "kuliner_kopi"
        ).score

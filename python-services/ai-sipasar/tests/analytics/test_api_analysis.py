...
from .conftest import SAMPLE_BUSINESS_PROFILE_ID, SAMPLE_PLACES_RESPONSE
...
class TestRunAnalysis:
    ...
    async def test_run_analysis_success(self, async_client) -> None:
        with patch("sipasar.services.competitor_service.PlacesProvider") as mock_cls:
            ...
            mock_provider.last_source = "mock"
        ...
        assert comp["count"] >= 0
        assert comp["data_source"] == "mock"
        assert comp["provider_status"] == "ok"
        assert comp["competitors"][0]["source"] == "google_places"
    ...
    @pytest.mark.asyncio
    async def test_run_analysis_returns_503_when_all_places_providers_fail(
        self, async_client
    ) -> None:
        with patch("sipasar.services.competitor_service.PlacesProvider") as mock_cls:
            mock_provider = mock_cls.return_value
            mock_provider.nearby_search = AsyncMock(
                side_effect=PlacesProviderUnavailableError("providers down")
            )
            mock_provider.aclose = AsyncMock()
            payload = {
                "business_profile_id": str(SAMPLE_BUSINESS_PROFILE_ID),
            ...
            response = await async_client.post("/v1/analysis/run", json=payload)

        assert response.status_code == 503
        assert response.json()["error"]["code"] == "COMPETITOR_PROVIDER_UNAVAILABLE"

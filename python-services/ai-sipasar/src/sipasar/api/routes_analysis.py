...
async def _run_pipeline(
    ...
    competitor_resp = CompetitorResponse(
        ...
        data_source=comp_result.data_source,
        provider_status=comp_result.provider_status,
    )
    ...
    logger.info(
        "Analysis %s completed: label=%s, score=%.3f", aid, score_result.label, score_result.score
    )
...
async def get_history(
    business_profile_id: Annotated[uuid.UUID, Query(description="UUID profil bisnis")],
) -> AnalysisHistoryResponse:
...
async def rerun_analysis(
    ...
    new_radius = (
        body.radius_meters if body and body.radius_meters else None
    ) or original.radius_meters

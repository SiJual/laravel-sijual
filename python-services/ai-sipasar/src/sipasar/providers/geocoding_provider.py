...
class GeocodingProvider:
    ...
    async def reverse_geocode(self, lat: float, lon: float) -> str:
        ...
        if self.settings.GOOGLE_GEOCODING_API_KEY:
            ...
            resp = await self._client.get(self.GOOGLE_URL, params=google_params)
            resp.raise_for_status()
            data = resp.json()
        ...
        # Nominatim reverse
        nominatim_params: dict[str, str | float] = {
            "lat": lat,
            "lon": lon,
            "format": "json",
        }
        resp = await self._client.get(
            "https://nominatim.openstreetmap.org/reverse", params=nominatim_params
        )
        resp.raise_for_status()
        data = resp.json()

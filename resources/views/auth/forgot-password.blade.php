<x-layouts.guest title="Lupa Kata Sandi — SiJual">
    <div class="min-h-screen flex items-center justify-center p-4 sm:p-6 bg-background font-body">
        <div class="w-full max-w-md bg-surface p-8 sm:p-10 rounded-xl border border-border shadow-card">
            ...
            <div class="mb-6">
                <h1 class="text-2xl font-bold font-display text-on-surface mb-1">Lupa Kata Sandi?</h1>
                <p class="text-sm font-medium text-on-surface-variant">Masukkan email Anda untuk menerima tautan pemulihan.</p>
            </div>

            @if (session('status'))
                <div class="mb-4 p-3 rounded-md bg-success-bg text-success text-xs font-semibold">
                    {{ session('status') }}
                </div>
            @endif
            ...
            <form action="{{ route('password.request') }}" method="POST" class="space-y-4">
                @csrf
                <div>
                    <label for="email" class="block text-xs font-semibold text-on-surface mb-1.5">Email Terdaftar</label>
                ...
                <button type="submit"
                        class="w-full py-3 px-4 bg-primary text-white font-semibold text-sm rounded-md shadow-card hover:bg-primary/90 transition">
                    Kirim Tautan Pemulihan
                </button>
            </form>

            <div class="mt-6 text-center text-xs font-semibold text-on-surface-variant">
                <a href="{{ route('login') }}" class="text-primary hover:underline">← Kembali ke halaman Masuk</a>
            </div>
        </div>
    </div>
</x-layouts.guest>
